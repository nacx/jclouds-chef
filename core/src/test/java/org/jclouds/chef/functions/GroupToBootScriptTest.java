...
public class GroupToBootScriptTest {
   ...
   public void testMustHaveValidatorName() {
      ...
      GroupToBootScript fn = new GroupToBootScript(Suppliers.ofInstance(URI.create("http://localhost:4000")),
            CacheLoader.from(Functions.forMap(ImmutableMap.<String, BootstrapConfig> of())), installChefGems,
   ...
   public void testMustHaveRunScriptsValue() {
      ...
      CacheLoader.from(Functions.forMap(ImmutableMap.<String, BootstrapConfig> of("foo", (BootstrapConfig) null))),
   ...
   public void testOneRecipe() throws IOException {
      Optional<PrivateKey> validatorCredential = Optional.of(createMock(PrivateKey.class));
      GroupToBootScript fn = new GroupToBootScript(Suppliers.ofInstance(URI.create("http://localhost:4000")),
            CacheLoader.from(Functions.forMap(ImmutableMap.<String, BootstrapConfig> of(
                  "foo",
                  BootstrapConfig.builder().runList(ImmutableList.of("recipe[apache2]", "role[webserver]"))
                        .attributes(new JsonBall("{\"tomcat6\":{\"ssl_port\":8433}}")).build()))), installChefGems,
            validatorName, validatorCredential);
   ...
   public void testOneRecipeAndEnvironment() throws IOException {
      Optional<PrivateKey> validatorCredential = Optional.of(createMock(PrivateKey.class));
      GroupToBootScript fn = new GroupToBootScript(Suppliers.ofInstance(URI.create("http://localhost:4000")),
            CacheLoader.from(Functions.forMap(ImmutableMap.<String, BootstrapConfig> of("foo", BootstrapConfig
                  .builder().environment("env").runList(ImmutableList.of("recipe[apache2]", "role[webserver]"))
                  .attributes(new JsonBall("{\"tomcat6\":{\"ssl_port\":8433}}")).build()))), installChefGems,
            validatorName, validatorCredential);
   ...
   public void testOneRecipeOmnibus() throws IOException {
      ...
      GroupToBootScript fn = new GroupToBootScript(Suppliers.ofInstance(URI.create("http://localhost:4000")),
            CacheLoader.from(Functions.forMap(ImmutableMap.<String, BootstrapConfig> of(
                  "foo",
                  BootstrapConfig.builder().runList(ImmutableList.of("recipe[apache2]", "role[webserver]"))
                        .attributes(new JsonBall("{\"tomcat6\":{\"ssl_port\":8433}}")).build()))), installChefOmnibus,
            validatorName, validatorCredential);
